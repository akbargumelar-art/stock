...
import { Plus, Search, Edit2, Trash2, X, AlertTriangle, Box, ChevronRight, FolderOpen } from "lucide-react";
import {
...
export default function CategoriesPage() {
    ...
    const [formData, setFormData] = useState({ name: "", prefix: "", description: "", parentId: 0 });
    ...
    const getHierarchicalCategories = () => {
        ...
        return parents.map(parent => ({
            ...parent,
            childCategories: categories.filter(c => c.parentId === parent.id),
        }));
    };

    const filteredCategories = () => {
        if (!search) return null; // Use hierarchy view when no search
    ...
    const handleOpenModal = (category?: CategoryWithCount) => {
        if (category) {
            ...
            setFormData({ name: "", prefix: "", description: "", parentId: 0 });
        }
        setShowModal(true);
    };
    ...
    const renderCategoryRow = (category: CategoryWithCount, isChild = false) => (
        <tr key={category.id} className="hover:bg-[var(--bg-tertiary)]/50 transition-colors">
            <td className="py-3 px-4 font-medium text-[var(--text-primary)]">
                <div className="flex items-center gap-2">
                    {isChild && (
                        <span className="text-[var(--text-muted)] ml-4 flex items-center gap-1">
        ...
        <div className="space-y-6 animate-fade-in pb-10">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-[var(--text-secondary)]">Manage product categories and sub-categories</p>
                </div>
                <button
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]">
                            {searchResults ? (
                                ...
                                searchResults.length === 0 ? (
                                    ...
                                    searchResults.map((category) => renderCategoryRow(category, !!category.parentId))
                                )
                            ) : (
                                // Hierarchical view
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">
                                    Parent Category
                                    <span className="text-xs text-[var(--text-muted)] ml-2">(Optional - leave empty for top-level)</span>
                                </label>
                                <select
                    ...
                    <div className="modal-content animate-slide-up p-6 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-[var(--text-secondary)] mb-6 text-sm">
                            ...
                            {(categoryToDelete?.children?.length || 0) > 0 && (
                                <span className="block mt-2 text-red-500 font-medium">
                                    Warning: This category has {categoryToDelete?.children?.length} sub-categories and cannot be deleted until they are moved or removed.
                                </span>
                            )}
                        </p>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                disabled={deleting || (categoryToDelete?._count.products || 0) > 0 || (categoryToDelete?.children?.length || 0) > 0}
